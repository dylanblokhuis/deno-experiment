...
export async function action(ctx: Context) {
  ...
  const { id } = await appRouterCaller(ctx).createOrUpdateFieldGroup({
    id: values.id,
    name: values.name,
    postTypes: Array.isArray(values.postTypes)
      ? values.postTypes
      : [values.postTypes],
...
export async function loader(ctx: Context) {
  ...
  const fieldGroup = await appRouterCaller(ctx).getFieldGroup({
    id: parseInt(id),
  });
...
export default function EditFieldGroups() {
  const { fieldTypes, fieldGroup, message, postTypes } = useLoaderData<
    typeof loader
  >();
  const actionData = useActionData<typeof action>();
  const [fields, setFields] = useState<Fields>(
    actionData?.values.fields || fieldGroup?.fields || [],
  );
    ...
    <div>
      ...
      >
        <div className="col-span-2 rounded-lg border bg-white p-5">
          ...
          {fieldGroup?.id && (
            <input type="hidden" name="id" value={fieldGroup?.id} />
          )}
          <Input
            label="Name"
            type="text"
          ...
          <table className="w-full">
            <thead>
              <tr>
                {["Order", "Label", "Name", "Type"].map((
                  title,
                ) => (
                  <th
                    key={title}
                    className="text-left py-2 px-4 bg-slate-200 first:rounded-l last:rounded-r"
                  >
                    {title}
                  </th>
                ))}
            ...
            <tbody className="py-5">
              {fields?.length === 0 && (
                <tr>
                  <td
                    colSpan={4}
                    className="text-center py-10 bg-slate-50"
                  >
                    No fields
                  </td>
                </tr>
            ...
            <tfoot>
              <tr>
                <td
                  className="py-2 px-2 bg-slate-200 rounded"
                  colSpan={4}
                >

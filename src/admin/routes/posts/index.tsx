...
export async function loader(ctx: Context) {
  const postTypeSlug = new URL(ctx.request.url).searchParams.get("postType") ||
    "post";
  const postType = await appRouterCaller(ctx).getPostType({
    slug: postTypeSlug,
  });
  ...
  return (
    <div>
      ...
      <a
        ...
        href={`/admin/posts/edit?postType=${postType.slug}`}
      >
        Add New
      </a>
          ...
          {posts.map((post) => (
            <tr key={post.id}>
              <td>
                <a
                  className="text-blue-500 underline"
                  href={`/admin/posts/edit?postType=${postType.slug}&id=${post.id}`}
                >
                  {post.title}
                </a>
              </td>

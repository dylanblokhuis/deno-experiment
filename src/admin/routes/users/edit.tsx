...
export async function loader(context: Context) {
  ...
  const id = new URL(context.request.url).searchParams.get("id") as
    | string
    | undefined;
  let user;
  if (id) {
...
export async function action(ctx: Context) {
  let id: string | undefined | number = new URL(ctx.request.url).searchParams
      ...
      >
        <div className="col-span-2 rounded-lg border bg-white p-5 gap-y-4 flex flex-col">
          ...
          <Input
            ...
            name="password"
            label="Password"
            placeholder={user && "Fill to change password"}
          />

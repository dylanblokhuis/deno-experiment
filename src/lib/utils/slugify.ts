/**
 * source https://deno.land/x/slugify@0.3.0/mod.ts?source - vendored because its used on the client
 * Replace the string with its slug counterpart
 * @param string String to slugify
 * @param options Additional options like replacement, remove or lower
 */
export function slugify(
  string: string,
  options:
    | { replacement?: string; remove?: string | RegExp; lower?: boolean }
    | string = {}
): string {
  const _options: { replacement?: string; remove?: string | RegExp; lower?: boolean } =
    typeof options === "string" ? { replacement: options } : options || {};

  const slug = string
    .split("")
    .reduce(
      (result: string, ch: string): string =>
        result +
        (charMap[ch] || ch)
          // allowed
          .replace(_options.remove || /[^\w\s$*_+~.()'"!\-:@]/g, ""),
      ""
    )
    // trim leading/trailing spaces
    .trim()
    // convert spaces
    .replace(/[-\s]+/g, _options.replacement || "-");

  return _options.lower ? slug.toLowerCase() : slug;
}
...
// const extend = function (customMap: { [key: string]: string }): void {
